...
- [JS 面试题](#js----)
    + [JS 原型链](#js----)
        * [构造方法](#----)
    + [typeof 和 instanceof 的区别](#typeof---instanceof----)
    + [数据类型](#----)
    + [NaN 指的是什么](#nan------)
    + [什么是包装对象](#-------)
    + [什么是作用域链](#-------)
    ...
    + [闭包的作用](#-----)
        ...
        * [闭包常见用途](#------)
        * [优缺点](#---)
    + [Let、Var 和 const 的区别](#let-var---const----)
        * [为什么 var 可以重复声明](#----var-------)
    + [数据属性 和 访问器属性的区别](#---------------)
    + [toString 和 valueOf](#tostring---valueof)
    + [箭头函数有没有 arguments 对象?](#--------arguments----)
    + [JS 处理二进制](#js------)
    + [异步有哪些解决方案](#---------)
    + [CO库](#co-)
    + [JS 事件循环](#js-----)
    + [CommonJS 中的 require/exports 和 ES6 中的 import/export 有何区别](#commonjs----require-exports---es6----import-export-----)
    + [一行代码实现数组去重](#----------)
    + [如何判断两个对象相等](#----------)
    + [前端性能优化](#------)
    + [Object.defineProperty(obj, prop, descriptor)](#objectdefineproperty-obj--prop--descriptor-)
    + [Set 与 数组 的区别](#set---------)
    + [Set 和 Map  的区别](#set---map-----)
    + [Object、Map 的区别](#object-map----)
    + [WeakMap 和 Map 的区别](#weakmap---map----)
    + [JS 防抖和节流 debounce Throttle](#js-------debounce-throttle)
      - [代码实现](#----)
    + [Promise](#promise)
        * [状态](#--)
        * [基本过程](#----)
      - [代码实现](#-----1)
        * [构造函数](#----)
        * [then 方法](#then---)
        * [Handler 函数封装存储 `onResolved` `onRejected` 函数和新生成 `promise` 对象](#handler---------onresolved---onrejected----------promise----)
        * [Resolve 函数](#resolve---)
    + [CommonJS / AMD / CMD / ES6](#commonjs---amd---cmd---es6)
      - [CommonJS](#commonjs)
      - [AMD 和 require.js](#amd---requirejs)
      - [CMD 和 sea.js](#cmd---seajs)
      - [ES6 Module](#es6-module)
    + [JavaScript 的四种继承方式](#javascript--------)
    + [ES6](#es6)
      - [ES6 在字符串层面做了哪些优化](#es6-------------)
      - [ES6 的 Proxy](#es6---proxy)
      - [ES6 的 Reflect](#es6---reflect)
      - [Iterator 是什么](#iterator----)
      - [Class / extends 有什么用](#class---extends-----)
      - [Generator](#generator)
        * [与 Iterator 接口的关系](#--iterator------)
        * [next 方法的参数](#next------)
      - [for...of](#forof)
      - [for...of 与 for...in 的区别](#forof---forin----)
    + [线程和 进程的区别](#---------)
    + [函数式编程](#-----)
...
JavaScript因其动态的特性，并不提供一个类似Java 或 C++一样的 class 实现。（ES6 中增加的Class 关键字只是语法糖，其本质还是依靠原型链实现）

JavaScript 继承的时候只有一种结构 —— 对象

每一个对象都有一个私有属性__proto__，指向这个对象的原型对象 prototype。该原型对象也有一个自己的原型对象 __proto__ 层层向上直到Object对象原型为 null。
